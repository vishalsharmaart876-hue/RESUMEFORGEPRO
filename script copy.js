  ...
  return `
    ...
    <div class="field-group">
      ...
      <div style="display:flex;gap:5px">
        <input type="password" id="groq-key" class="field-input" placeholder="Paste your Groq API key (gsk_...)" value="${localStorage.getItem('rf_api_key') || ''}">
        <button class="btn btn-secondary btn-sm" onclick="saveApiKey()">Save</button>
      </div>
      <div id="api-status" style="font-size:9px;margin-top:4px;color:${localStorage.getItem('rf_api_key') ? 'var(--green)' : 'var(--text3)'}">
        ${localStorage.getItem('rf_api_key') ? '● AI Active (Groq API Mode)' : '○ AI Active (Local Mode)'}
      </div>
    </div>
...
async function callGroqAPI(msg, typingId) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (data.error?.message?.includes("API key not valid") || data.error?.code === "invalid_api_key") {
        errorMsg += "Your Groq API key is invalid. Please double-check it in Settings.";
      } else if (data.error?.status === "PERMISSION_DENIED") {
        errorMsg += "Access denied. Ensure the Groq API is active.";
      } else {
    ...
    if (data.choices && data.choices[0].message.content) {
      ...
      if (el) {
        // Basic Markdown-ish simple replacement (strong and br)
    ...
    if (el) {
      ...
      1. **Invalid Groq API Key** (Check Settings)<br>
      2. **Ad-Blocker/Work VPN** blocking Groq services<br>
